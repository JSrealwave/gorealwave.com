...
import {
  SOCAL_REALWAVE_TRNG_DECK_SLIDES,
} from "@/lib/one-pager/socal-realwave-trng-deck-slides";
import {
  onePagerPdfAttrs,
  onePagerRoot,
  PDF_CLASSES,
...
const SLUG = "socal-realwave-trng-deck" as const;
const SLIDE_ASPECT = "aspect-[2485/1920]";

type SocalRealwaveTrngDeckProps = {
  pdfMode?: boolean;
  pdfClean?: boolean;
...
export function SocalRealwaveTrngDeck({
  pdfMode = false,
  pdfClean = false,
  showDownloadInPdfMode = false,
  initialSlide = null,
}: SocalRealwaveTrngDeckProps) {
  const slides = SOCAL_REALWAVE_TRNG_DECK_SLIDES;
  ...
  return (
    ...
    >
      <OnePagerToolbar
        pdfSlug={SLUG}
        downloadFilename="socal-realwave-trng-deck.pdf"
        label="SoCal Realwave Training Deck"
        pdfMode={pdfMode}
        pdfClean={pdfClean}
        ...
        >
          ...
          >
            ...
            <h1 className="mb-3 max-w-3xl text-3xl font-semibold leading-[1.1] tracking-[-1px] sm:text-4xl">
              SoCal Realwave Training
            </h1>
            <p className="max-w-2xl text-[15px] leading-relaxed text-white/85 sm:text-lg">
              {pdfMode
                ? "Full SoCal Realwave training deck for partner enablement sessions."
                : "Open the interactive viewer to step through the training deck with thumbnails, or download a PDF copy to share."}
            </p>
          ...
          {!pdfMode ? (
            <div className="bg-slate-50 px-5 py-8 sm:px-8 sm:py-10">
              ...
              >
                <div className={cn("w-full", SLIDE_ASPECT)}>
                  <img
                    src={cover.src}
                    alt={cover.alt}
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:thin]">
                  {slides.map((slide, i) => (
                    <button
                      ...
                      type="button"
                      onClick={() => openDeck(i)}
                      className="relative h-[56px] w-[72px] shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-white transition hover:border-[#12498a] hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00d4a6]/50"
              ...
              {slides.map((slide, index) => (
                ...
                >
                  ...
                  <div className="bg-[#0b1220] p-2">
                    <div className={cn("w-full overflow-hidden rounded-md", SLIDE_ASPECT)}>
                      <img
                        src={slide.src}
                        alt={slide.alt}
      ...
      {!pdfMode ? (
        <>
          <DeckModal
            ...
            deckTitle="SoCal Realwave Training Deck"
            syncUrl
            slideAspectClass={SLIDE_ASPECT}
          />
